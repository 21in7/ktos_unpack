...
function ITEM_TOOLTIP_ETCSEAL(tooltipframe, invitem, argStr, usesubframe)	
	...
	if descRichtext ~= nil then
		...
		desc = replace(desc, 'name', shared_item_jurate_seal.get_seal_name(invitem))
		desc = replace(desc, 'name', shared_item_jurate_seal.get_seal_name(invitem))
		desc = desc .. shared_item_jurate_seal.get_consume_seal_name(invitem) .. '{nl}'
		...
		for k, v in pairs(cost_list) do
			...
			local msg = string.format('- %s %s{nl}', mat_cls.Name, GET_COMMAED_STRING(v))
			desc = desc .. msg
		end
		descRichtext:SetText(desc)
	end
